...
// Karten sind nur klickbar wenn: dieser Spieler dran ist UND es die lokale Zone ist
const isPlayerActive = computed(() => {
  const isActive = playerIndex === gameTable.value.turnStats.activePlayerIndex
  const isLocal = localPlayerIndex.value === -1 || localPlayerIndex.value === playerIndex
  return isActive && isLocal
})
</script>
<style lang="scss">
.cardsContainer {
